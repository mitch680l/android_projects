...
class TicTacToeViewModel: ViewModel() {
    ...
    fun setButton(id: Int) {
        ...
        if (_state.value.victor.equals(null)) {
    ...
    private fun isGameOver(): Boolean {
        if(rowHasWinner(1) || rowHasWinner(2) || rowHasWinner(3)){return true}
        else if(columnHasWinner(1) || columnHasWinner(2) || columnHasWinner(3)) {return true}
        else if(firstDiagonalHasWinner() || secondDiagonalHasWinner()){return true}
        else {
            return false
        }
    }
    ...
    private fun compareSpaces(first: Int, second: Int, third: Int): Boolean{
        return if(_state.value.buttonValues[first] == "-") {
            ...
            _state.value.buttonValues[first] == _state.value.buttonValues[second]
            && _state.value.buttonValues[second] == _state.value.buttonValues[third]
        ) {
